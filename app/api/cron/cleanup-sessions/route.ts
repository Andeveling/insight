  ...
  try {
    ...
    for (let i = 0; i < staleSessions.length; i += batchSize) {
      ...
      try {
        ...
        console.log(`[Session Cleanup] Archived batch ${i / batchSize + 1}: ${result.count} sessions`);
      } catch (error) {
        failed += batch.length;
 ...
 */
async function deleteOldArchivedSessions(daysArchived: number = 90): Promise<number> {
  const cutoffDate = new Date();
  ...
  try {
    ...
    console.log(`[Session Cleanup] Deleted ${result.count} old archived sessions`);
    return result.count;
  } catch (error) {
