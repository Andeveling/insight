...
const clipPath15 = "polygon(15px 0, 100% 0, 100% calc(100% - 15px), calc(100% - 15px) 100%, 0 100%, 0 15px)";
const clipPath8 = "polygon(8px 0, 100% 0, 100% calc(100% - 8px), calc(100% - 8px) 100%, 0 100%, 0 8px)";
const clipPath20 = "polygon(20px 0, 100% 0, 100% calc(100% - 20px), calc(100% - 20px) 100%, 0 100%, 0 20px)";
	...
	return (
		<div className="flex min-h-screen flex-col bg-background text-foreground selection:bg-primary/30 relative">
				...
				<div className="container mx-auto flex h-16 items-center justify-between px-4 md:px-6">
					<div className="flex items-center gap-3 group cursor-pointer">
						...
						<div className="flex flex-col -space-y-1">
							<span className="font-black text-xl tracking-tighter uppercase">Insight</span>
							<span className="text-[8px] font-black tracking-[0.4em] text-primary/60">SYSTEM_v4.2</span>
						</div>
					</div>
					<nav className="flex items-center gap-6">
						<div className="hidden md:flex items-center gap-6 text-[10px] font-black uppercase tracking-widest text-muted-foreground/60">
							...
							<Link href="#stats" className="hover:text-primary transition-colors">Métricas</Link>
						</div>
						<Link href="/login">
							<button 
			...
			<main className="flex-1 relative z-10">
						...
						<div className="flex flex-col items-center space-y-12 text-center">
							...
							>
								...
								<h1 className="text-4xl font-black tracking-tighter sm:text-5xl md:text-6xl lg:text-8xl/none uppercase">
									Potencia el <span className="text-primary italic">Core</span> <br />
									de tu <span className="relative">
										Equipo
										<svg className="absolute -bottom-2 md:-bottom-4 left-0 w-full h-2 md:h-4 text-primary/30" viewBox="0 0 100 20" preserveAspectRatio="none">
								...
								<p className="mx-auto max-w-[800px] text-muted-foreground/80 md:text-lg font-medium leading-relaxed uppercase tracking-wide text-xs">
									[ANALIZANDO_FORTALEZAS] // Insight utiliza inteligencia artificial para decodificar
					...
					<div className="container mx-auto px-4 md:px-6 mb-20 relative z-10">
						<div className="flex flex-col items-center justify-center space-y-4 text-center">
							<div className="space-y-2">
								<div className="text-xs font-black uppercase tracking-[0.4em] text-primary/60 mb-2">Módulos_Operativos</div>
								<h2 className="text-3xl font-black tracking-tighter md:text-5xl uppercase">
									Arquitectura de <span className="text-primary">Liderazgo</span>
								</h2>
								<p className="max-w-[600px] text-muted-foreground/60 text-[10px] font-bold uppercase tracking-widest mx-auto">
									Infraestructura de datos diseñada para la optimización de capital humano_
								</p>
							</div>
						</div>
				...
				<section id="stats" className="w-full py-24 md:py-32 border-y border-white/5 relative bg-background">
					<div className="container mx-auto px-4 md:px-6">
						<div className="grid gap-16 lg:grid-cols-2 items-center">
							<div className="space-y-8">
								<div className="space-y-4">
									<h2 className="text-3xl font-black tracking-tighter md:text-5xl uppercase">
										Optimiza la <span className="text-primary">Cultura</span> Organica
									</h2>
									<p className="text-muted-foreground/80 md:text-lg font-medium leading-relaxed uppercase tracking-wide text-xs">
										Insight no es hardware redundante; es el núcleo para construir escuadrones 
										de alto impacto mediante analítica de comportamiento_
									</p>
								</div>
								
								<div className="grid gap-4">
									<TechList icon={<Target />} text="Sincronización de comunicación interna" />
									<TechList icon={<CheckCircle2 />} text="Compromiso verificado vía protocolos IA" />
									<TechList icon={<TrendingUp />} text="Escalabilidad basada en métricas de fortalezas" />
									<TechList icon={<History />} text="Historial de evolución de equipos dinámico" />
								</div>
				...
				<section id="development" className="w-full py-24 md:py-32 bg-muted/5 relative overflow-hidden border-t border-white/5">
					<div className="container mx-auto px-4 md:px-6 relative z-10">
						<div className="flex flex-col items-center justify-center mb-16 text-center">
							<div className="inline-flex items-center gap-2 px-3 py-1 bg-primary/10 border border-primary/20 text-[10px] font-black uppercase tracking-[0.3em] text-primary mb-4" style={{ clipPath: clipPath8 }}>
								<Sparkles className="size-3" />
								Nuevo: Módulo_de_Desarrollo
							</div>
							<h2 className="text-3xl font-black tracking-tighter md:text-5xl uppercase mb-4">
								Laboratorio de <span className="text-primary italic">Evolución</span>
							</h2>
							<p className="max-w-[700px] text-muted-foreground/60 text-xs font-bold uppercase tracking-widest leading-relaxed">
								No solo identifiques fortalezas; desarróllalas mediante un sistema de aprendizaje adaptativo basado en IA_
							</p>
						</div>

						<div className="grid gap-12 md:grid-cols-2 lg:grid-cols-4">
							<div className="space-y-4 group">
								<div className="w-12 h-12 flex items-center justify-center bg-primary/10 text-primary border border-primary/20 transition-all group-hover:bg-primary group-hover:text-primary-foreground" style={{ clipPath: "polygon(50% 0%, 100% 25%, 100% 75%, 50% 100%, 0% 75%, 0% 25%)" }}>
									<BookOpen className="size-6" />
								</div>
								<h3 className="text-sm font-black uppercase tracking-tight">Módulos On-Demand</h3>
								<p className="text-[11px] text-muted-foreground leading-relaxed">Generación instantánea de micro-learning adaptados a tu rol profesional y tus Top 5 fortalezas.</p>
							</div>

							<div className="space-y-4 group">
								<div className="w-12 h-12 flex items-center justify-center bg-chart-2/10 text-chart-2 border border-chart-2/20 transition-all group-hover:bg-chart-2 group-hover:text-white" style={{ clipPath: "polygon(50% 0%, 100% 25%, 100% 75%, 50% 100%, 0% 75%, 0% 25%)" }}>
									<Trophy className="size-6" />
								</div>
								<h3 className="text-sm font-black uppercase tracking-tight">Gamificación Gallup</h3>
								<p className="text-[11px] text-muted-foreground leading-relaxed">Gana XP, desbloquea Badges de Maestría y escala en los rankings de liderazgo de tu organización.</p>
							</div>

							<div className="space-y-4 group">
								<div className="w-12 h-12 flex items-center justify-center bg-amber-500/10 text-amber-500 border border-amber-500/20 transition-all group-hover:bg-amber-500 group-hover:text-black" style={{ clipPath: "polygon(50% 0%, 100% 25%, 100% 75%, 50% 100%, 0% 75%, 0% 25%)" }}>
									<BrainCircuit className="size-6" />
								</div>
								<h3 className="text-sm font-black uppercase tracking-tight">AI Coach 24/7</h3>
								<p className="text-[11px] text-muted-foreground leading-relaxed">Recibe directivas diarias y consejos tácticos para aplicar tus fortalezas en situaciones reales de trabajo.</p>
							</div>

							<div className="space-y-4 group">
								<div className="w-12 h-12 flex items-center justify-center bg-emerald-500/10 text-emerald-500 border border-emerald-500/20 transition-all group-hover:bg-emerald-500 group-hover:text-black" style={{ clipPath: "polygon(50% 0%, 100% 25%, 100% 75%, 50% 100%, 0% 75%, 0% 25%)" }}>
									<TrendingUp className="size-6" />
								</div>
								<h3 className="text-sm font-black uppercase tracking-tight">Métricas de Crecimiento</h3>
								<p className="text-[11px] text-muted-foreground leading-relaxed">Visualiza tu evolución técnica y blanda mediante gráficos HUD de alto impacto y reportes de progreso.</p>
							</div>
						</div>
			...
			<footer className="w-full border-t border-white/5 py-12 relative z-10 bg-background">
				<div className="container mx-auto flex flex-col items-center justify-between gap-8 md:flex-row px-4 md:px-6">
					<div className="flex items-center gap-3">
						...
						<div className="flex flex-col -space-y-1">
							<span className="font-black text-lg tracking-tighter uppercase">Insight</span>
							<span className="text-[7px] font-black tracking-[0.4em] text-primary/40 text-center">CORE</span>
						</div>
					</div>
			...
			>
				...
				>
					...
					<div className="absolute top-4 right-4 flex items-center gap-1.5">
						...
						<span className="text-[6px] font-black uppercase tracking-widest text-emerald-500/60">Active_Node</span>
					</div>
					...
					<h3 className="text-lg font-black uppercase tracking-tighter text-foreground group-hover:text-primary transition-colors">{title}</h3>
