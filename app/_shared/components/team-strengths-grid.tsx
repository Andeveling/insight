...
export function TeamStrengthsGrid({
  ...
  const strengthsByDomain = useMemo(() => {
    const allStrengths = teamMembers.flatMap((member) =>
      member.strengths.map((s) => s.strength)
    );

    // Remove duplicates
    const uniqueStrengths = Array.from(
      new Map(allStrengths.map((s) => [s.id, s])).values()
    );
  ...
  const orderedStrengths = useMemo(() => {
    const domains: DomainType[] = ["Doing", "Feeling", "Motivating", "Thinking"];
  ...
  return (
    <Card className={cn("w-full overflow-hidden", className)}>
      <CardHeader>
        <CardTitle className="text-2xl">Matriz de Fortalezas del Equipo</CardTitle>
        ...
        <div className="overflow-x-auto">
          <div className="min-w-max">
            ...
            <div className="flex border-b bg-muted/50">
              ...
              {(["Doing", "Feeling", "Motivating", "Thinking"] as DomainType[]).map(
                (domain) => {
            ...
            <div className="flex border-b bg-background sticky top-0 z-10">
              <div className="sticky left-0 z-20 w-48 bg-background border-r" />
              {orderedStrengths.map((strength) => (
                <TooltipProvider key={strength.id}>
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <div
                        className="w-12 p-2 text-center border-l cursor-help"
                        style={{
                          backgroundColor: getDomainColor(strength.domain, "bg"),
                        }}
                      >
                        <div
                          className="text-xs font-medium truncate transform -rotate-45 origin-bottom-left whitespace-nowrap"
                          style={{
            ...
            {teamMembers.map((member, idx) => (
              ...
              >
                {/* Member Name */}
                <div className="sticky left-0 z-10 w-48 bg-inherit border-r p-3 flex items-center gap-2">
                ...
                {orderedStrengths.map((strength) => {
                  ...
                  return (
                    <div
                      key={strength.id}
                      className="w-12 p-2 flex items-center justify-center border-l"
                    >
                      {hasStrength && (
                        <TooltipProvider>
                          <Tooltip>
