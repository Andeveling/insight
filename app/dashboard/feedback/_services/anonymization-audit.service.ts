...
async function checkAnonymousHashPresence(): Promise<AuditCheckResult> {
	...
	if (anonymousRequestsWithoutHashes > 0) {
		return {
			...
			recommendation:
				"Generate hashes for existing anonymous responses via migration",
		};
	}
...
async function checkHashFormat(): Promise<AuditCheckResult> {
	...
	for (const response of sampleResponses) {
		if (
			response.anonymousHash &&
			!isValidAnonymousHash(response.anonymousHash)
		) {
			invalidHashes++;
		}
	}
  ...
  `;
	...
	if (duplicateCount > 0) {
		return {
			...
			recommendation:
				"Investigate duplicate hashes for potential duplicate detection issues",
		};
	}
...
async function checkSummaryAnonymity(): Promise<AuditCheckResult> {
	...
	for (const summary of summaries) {
		const text =
			`${summary.insights || ""} ${summary.strengthAdjustments}`.toLowerCase();
	...
	if (exposures > 0) {
		return {
			...
			recommendation:
				"Review AI prompts to ensure they do not include respondent names",
		};
	}
...
async function checkHashConsistency(): Promise<AuditCheckResult> {
	...
	const testInputs = [
		{
			respondentId: "test-user-1",
			requestId: "test-request-1",
			questionId: "test-q-1",
		},
		{
			respondentId: "test-user-2",
	...
	if (inconsistent > 0) {
		return {
			...
			recommendation:
				"Review hash generation algorithm for non-deterministic elements",
		};
	}
...
export function logAuditResults(report: AnonymizationAuditReport): void {
	...
	console.log(
		`Critical: ${report.criticalIssues} | High: ${report.highIssues} | Medium: ${report.mediumIssues} | Low: ${report.lowIssues}`,
	);
	console.log("\nCheck Details:");
	...
	for (const check of report.checks) {
		...
		console.log(
			`  ${color}${icon}${reset} [${check.severity.toUpperCase()}] ${check.check}`,
		);
