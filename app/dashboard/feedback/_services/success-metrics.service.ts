 ...
 */
async function calculateSC002(): Promise<{
	avg: number | null;
	passing: boolean;
}> {
	...
	const avgMinutes =
		completionMinutes.reduce((a, b) => a + b, 0) / completionMinutes.length;
 ...
 */
async function calculateSC007(): Promise<{
	breaches: number;
	passing: boolean;
}> {
...
export async function calculateSuccessMetrics(): Promise<SuccessMetrics> {
	...
	const passingCriteria = measuredCriteria.filter(
		(c) => c.passing === true,
	).length;
...
export function logSuccessMetrics(metrics: SuccessMetrics): void {
	...
	console.log(
		`Overall: ${metrics.passingCriteria}/${metrics.totalCriteria} criteria passing (${Math.round(metrics.overallScore)}%)\n`,
	);
	...
	formatCriteria(
		"SC-001 Request Flow Time",
		metrics.sc001_avgRequestFlowTime,
		2,
		true,
		" min",
	);
	formatCriteria(
		"SC-002 Response Completion Time",
		metrics.sc002_avgResponseCompletionTime,
		metrics.sc002_target,
		metrics.sc002_passing,
		" min",
	);
	formatCriteria(
		"SC-004 Insight Generation Time",
		metrics.sc004_avgInsightGenerationTime,
		metrics.sc004_target,
		metrics.sc004_passing,
		" sec",
	);

	console.log("\n=== Engagement ===");
	formatCriteria(
		"SC-003 Response Rate",
		metrics.sc003_responseRate,
		metrics.sc003_target,
		metrics.sc003_passing,
		"%",
	);
	formatCriteria(
		"SC-005 Profile Adjustment Rate",
		metrics.sc005_adjustmentRate,
		metrics.sc005_target,
		metrics.sc005_passing,
		"%",
	);
	formatCriteria(
		"SC-009 Completion Rate",
		metrics.sc009_completionRate,
		metrics.sc009_target,
		metrics.sc009_passing,
		"%",
	);

	console.log("\n=== Quality & Security ===");
	formatCriteria(
		"SC-006 Notification Delivery",
		metrics.sc006_notificationDeliveryRate,
		metrics.sc006_target,
		metrics.sc006_passing,
		"%",
	);
	console.log(
		`${metrics.sc007_passing ? "✅" : "❌"} SC-007 Anonymity Breaches: ${metrics.sc007_anonymityBreaches} (target: ${metrics.sc007_target})`,
	);
	formatCriteria(
		"SC-008 Satisfaction Score",
		metrics.sc008_satisfactionScore,
		metrics.sc008_target,
		metrics.sc008_passing,
		"/5",
	);
