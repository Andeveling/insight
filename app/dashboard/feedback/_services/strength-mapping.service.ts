...
export function aggregateStrengthScores(
  ...
  for (const [ strengthKey, weights ] of strengthScores) {
    ...
    const normalizedScore = maxPossible > 0
      ? Math.min(100, (totalWeight / maxPossible) * 100)
      : 0;
...
export async function analyzePeerFeedback(
  ...
  const topStrengths = aggregatedScores
    .slice(0, 5)
    .map((s) => s.strengthKey);
...
export async function compareWithSelfAssessment(
  ...
  const peerObservedKeys = new Set(
    feedbackAnalysis.topStrengths
  );
  ...
  const validated = feedbackAnalysis.topStrengths.filter(
    (key) => selfAssessedKeys.has(key)
  );
