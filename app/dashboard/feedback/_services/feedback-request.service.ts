...
export async function createFeedbackRequests(
  ...
  if (respondentIds.length < 3 || respondentIds.length > 5) {
    return {
      success: false,
      createdRequests: [],
      errors: [ { respondentId: '', reason: 'Must select between 3 and 5 teammates' } ],
    };
  }
  ...
  if (respondentIds.includes(requesterId)) {
    return {
      success: false,
      createdRequests: [],
      errors: [ { respondentId: requesterId, reason: 'Cannot request feedback from yourself' } ],
    };
  }
  ...
  if (uniqueRespondents.size !== respondentIds.length) {
    return {
      success: false,
      createdRequests: [],
      errors: [ { respondentId: '', reason: 'Duplicate respondents in request' } ],
    };
  }
  ...
  for (const respondentId of respondentIds) {
    ...
    try {
      ...
      console.error(`Failed to create feedback request for ${respondentId}:`, error);
