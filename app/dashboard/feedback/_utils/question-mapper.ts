...
export function parseFeedbackQuestion(
	...
	return {
		...
		answerOptions: JSON.parse(
			question.answerOptions as string,
		) as AnswerOption[],
		strengthMapping: JSON.parse(
			question.strengthMapping as string,
		) as StrengthMapping,
	};
}
...
export function getStrengthWeightsForAnswer(
	...
	if (!mapping) {
		console.warn(
			`No strength mapping found for answer "${answerId}" in question "${question.id}"`,
		);
		return [];
	}
 ...
 */
export function getAffectedStrengths(
	question: ParsedFeedbackQuestion,
): string[] {
