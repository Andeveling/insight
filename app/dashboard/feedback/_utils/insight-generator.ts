...
export function buildInsightPrompt(input: InsightGenerationInput): string {
	...
	const topPeerStrengths = peerStrengths
		...
		.map(
			(s) =>
				`- ${s.strengthKey}: ${s.normalizedScore.toFixed(1)}% (${s.responseCount} observaciones)`,
		)
		.join("\n");
...
export function generateRuleBasedInsights(
	...
	const topPeerKeys = peerStrengths
		.slice(0, 5)
		.map((s) => s.strengthKey.toLowerCase());
...
function generateRuleBasedSummary(
	...
	if (agreements.length > 0) {
		const names = agreements
			.map((i) => formatStrengthName(i.strengthKey))
			.join(", ");
		parts.push(
