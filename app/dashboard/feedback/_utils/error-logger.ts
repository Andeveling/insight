 ...
 */
function determineSeverity(error: unknown, context: AIErrorContext): ErrorSeverity {
...
export function logAIError(
  ...
  console.error(`${color}[AI Error ${calculatedSeverity.toUpperCase()}]${reset} ${errorId}`);
  ...
  if (process.env.NODE_ENV === 'production' && calculatedSeverity === 'critical') {
 ...
 */
function sanitizeInputSummary(input: Record<string, unknown>): Record<string, unknown> {
  const sanitized: Record<string, unknown> = {};

  for (const [ key, value ] of Object.entries(input)) {
    // Skip potentially sensitive keys
    if ([ 'email', 'password', 'token', 'secret', 'key' ].some((s) => key.toLowerCase().includes(s))) {
...
function sendToErrorTrackingService(error: LoggedError): void {
  ...
  console.log(`[ErrorTracking] Would send error ${error.id} to tracking service`);
}
...
export function logAISuccess(context: {
  ...
  if (process.env.NODE_ENV === 'development') {
    console.log(`[AI Success] ${context.operation} completed in ${context.durationMs}ms`);
