...
export const RedFlagSchema = z.object({
	...
	severity: z
		...
		.describe(
			"low=bajo, medium=medio, high=alto - qué tan crítico es este riesgo",
		),
	description: z
		.string()
...
export const BlindSpotSchema = z.object({
	strengthName: z
		.string()
		.describe("La fortaleza que puede volverse punto ciego"),
	darkSide: z
		.string()
		.describe(
			"Cómo esta fortaleza puede convertirse en debilidad cuando se usa en exceso",
		),
	triggers: z
		.array(z.string())
...
export const DevelopmentStrategySchema = z.object({
	...
	currentLevel: z
		...
		.describe(
			"emerging=emergente, developing=en desarrollo, established=establecido, mastery=maestría",
		),
	shortTermActions: z
		.array(z.string())
	...
	longTermGoals: z
		.array(z.string())
		.describe("Metas para los próximos 6-12 meses"),
	resources: z
		.array(z.string())
...
export const IndividualReportSchema = z.object({
	...
	strengthsDynamics: z.object({
		...
		uniqueBlend: z
			.string()
			.describe(
				"Descripción de qué hace esta combinación específica especial y rara",
			),
	}),
	...
	actionPlan: z.object({
		immediate: z.array(z.string()).describe("Acciones a tomar esta semana"),
		shortTerm: z.array(z.string()).describe("Metas para el próximo mes"),
		longTerm: z.array(z.string()).describe("Visión para el próximo año"),
	}),
});
