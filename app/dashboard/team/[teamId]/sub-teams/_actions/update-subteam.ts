...
export async function updateSubTeamAction(
  ...
  try {
    ...
    if (!isCreator && !isAdmin) {
      return {
        success: false,
        error: 'Solo el creador o un administrador puede editar este sub-equipo',
      };
    }
    ...
    if (validatedData.members && validatedData.members.length > 0) {
      ...
      const invalidMembers = validatedData.members.filter((id) => !validMemberIds.has(id));
    ...
    revalidatePath(`/dashboard/team/${existingSubTeam.parentTeamId}/sub-teams/${result.id}`);
    ...
    return {
      success: false,
      error: error instanceof Error ? error.message : 'Error al actualizar el sub-equipo',
    };
  }
}
