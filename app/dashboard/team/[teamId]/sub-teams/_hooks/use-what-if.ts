...
export function useWhatIf({
  ...
  const swapMember = useCallback(
    async (removeId: string, addId: string) => {
      ...
      startTransition(async () => {
        ...
        const previousScore = state.simulatedResult?.totalScore ?? state.originalScore ?? 0;
    ...
    [ state.isActive, state.simulatedMembers, state.simulatedResult, state.originalScore, recalculateScore, getMemberName ]
  );
  ...
  }, [ state.isActive, state.pendingSwaps, state.simulatedMembers, recalculateScore ]);
  ...
  const getScoreDelta = useCallback((): number => {
    const currentScore = state.simulatedResult?.totalScore ?? state.originalScore ?? 0;
