...
import type { AssessmentQuestion, AnswerValue, QuestionType } from '@/lib/types/assessment.types';
...
export function useQuestionNavigation({
  ...
  const isValid = useMemo(() => {
    ...
    if (isScale) return typeof answer === 'number' && answer >= 1 && answer <= 5;
    if (isChoice) return typeof answer === 'string' && answer.trim() !== '';
    if (isRanking) return Array.isArray(answer) && answer.length === options.length;
    return false;
