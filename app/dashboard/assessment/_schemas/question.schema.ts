...
export const validateQuestionConsistency = (question: z.infer<typeof AssessmentQuestionSchema>) => {
  ...
  if ((question.type === 'CHOICE' || question.type === 'RANKING') && !question.options) {
    throw new Error('Choice and Ranking questions must have options defined');
  }

  // Ranking questions must have at least 3 options
  if (question.type === 'RANKING' && question.options && question.options.length < 3) {
...
export type AssessmentQuestionValidated = z.infer<typeof AssessmentQuestionSchema>;
