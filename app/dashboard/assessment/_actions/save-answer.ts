...
import type { AnswerValue, AssessmentQuestion } from '@/lib/types/assessment.types';
...
export async function saveAnswer(input: SaveAnswerInput): Promise<SaveAnswerResult> {
  try {
    ...
    await prisma.assessmentSession.update({
      where: { id: sessionId },
      data: {
        currentStep: newCurrentStep > assessmentSession.totalSteps
    ...
    if (!phaseComplete) {
      ...
      if (nextQ) {
        nextQuestion = {
          ...
          scaleRange: nextQ.scaleRange ? JSON.parse(nextQ.scaleRange) : undefined,
