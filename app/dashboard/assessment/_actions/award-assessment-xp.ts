...
export async function awardAssessmentXp(
	...
	try {
		...
		const xpAwarded: XpAwardedTracking =
			(resultsData.xpAwarded as XpAwardedTracking) ?? {};
 ...
 */
export async function getAssessmentXpStatus(sessionId: string): Promise<{
	...
	try {
		...
		if (
			!assessmentSession ||
			assessmentSession.userId !== userSession.user.id
		) {
			return null;
		}
