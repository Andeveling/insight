...
export async function getUserProgress(): Promise<UserProgressResult> {
  ...
  const modulesCompleted =
    (moduleProgress as ModuleProgressEntry[]).find((p) => p.status === "completed")?._count.id ?? 0;
  const modulesInProgress =
    (moduleProgress as ModuleProgressEntry[]).find((p) => p.status === "in_progress")?._count.id ?? 0;
