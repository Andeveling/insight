...
import { type CollaborativeChallengeStatus, type ChallengeCompletionResult } from "../_schemas";
import { calculateXpUpdate } from "@/lib/services/xp-calculator.service";
import { getNewlyUnlockedBadges, type UserBadgeStats } from "@/lib/constants/badge-criteria";
 ...
 */
export async function initiateCollaborativeChallenge(
  input: { challengeId: string; partnerId: string }
...
export async function confirmCollaborativeChallenge(
  ...
  if (collaborative.status === "expired" || new Date() > collaborative.expiresAt) {
  ...
  const partnerResult = await prisma.$transaction(async (tx) => {
    ...
    const partnerXpResult = calculateXpUpdate(partnerGamification.xpTotal, xpWithBonus);
  ...
  const newBadgeKeys = getNewlyUnlockedBadges(allBadges, unlockedKeys, userStats);
