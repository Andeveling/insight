...
async function saveAppliedMigrations(migrations: AppliedMigrations): Promise<void> {
...
async function main() {
  ...
  console.log(`✅ ${appliedMigrations.applied.length} already applied to Turso`)
  ...
  if (specificMigration) {
    ...
    pendingMigrations = allMigrations.filter(m => !appliedMigrations.applied.includes(m))
  ...
  for (const migration of pendingMigrations) {
    ...
    try {
      for (let i = 0; i < statements.length; i++) {
        ...
        try {
          ...
          if (
            ...
            errorMessage.includes('SQLITE_ERROR') && stmt.includes('CREATE TABLE IF NOT EXISTS')
          ) {
            console.log(`   ⚠️  Skipping (already exists): ${stmt.substring(0, 50)}...`)
            continue
          }
          throw stmtError
        }
      ...
      console.log(`      echo '{"applied":${JSON.stringify([ ...appliedMigrations.applied, migration ])},"lastApplied":"${migration}"}' > .turso-migrations.json`)
