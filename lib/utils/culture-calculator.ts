...
export function determineCulture(focusScores: FocusScores): CultureAnalysis {
	...
	const dominantEnergy: FocusType =
		action >= reflection ? "Action" : "Reflection";
	const dominantOrientation: FocusType =
	...
	if (confidence < 0.3) {
		...
		const secondEnergy: FocusType =
			dominantEnergy === "Action" ? "Reflection" : "Action";
		const secondOrientation: FocusType =
			dominantOrientation === "Results" ? "People" : "Results";
