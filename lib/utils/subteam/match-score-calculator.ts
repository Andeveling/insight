 ...
 */
export function getMatchScoreCacheStats(): {
	size: number;
	maxSize: number;
	ttlMs: number;
} {
...
export function calculateMatchScore(
	...
	const strengthCoverage = calculateStrengthCoverage(
		members,
		projectType.idealStrengths,
	);
	const domainBalance = calculateDomainBalance(
		members,
	...
	const totalScore = Math.max(
		0,
		Math.min(100, Math.round(positiveScore - redundancyPenalty.contribution)),
	);
...
export function calculateCultureFit(
	...
	// Calculate match
	const matchCount = idealCultures.filter((c) =>
		teamCultures.includes(c),
	).length;
	const matchPercentage =
		idealCultures.length > 0 ? matchCount / idealCultures.length : 0;
...
export function calculateRedundancyPenalty(
	...
	members.forEach((member) => {
		member.strengths.forEach((strength) => {
			const existing = strengthCounts.get(strength.name) || {
				count: 0,
				members: [],
			};
			existing.count++;
	...
	const totalDuplicates = duplicateStrengths.reduce(
		(sum, d) => sum + (d.count - 1),
		0,
	);
