...
export function selectPhase2Questions(
	...
	console.log(
		"[AdaptiveLogic] Top domains selected:",
		topDomains.map((d) => `${d.domainName}: ${d.score.toFixed(1)}%`).join(", "),
	);

	// Handle edge cases
	const edgeCaseResult = handleEdgeCases(
		domainScores,
		topDomains,
		questionBank,
	);
	if (edgeCaseResult) {
		console.log(
			"[AdaptiveLogic] Edge case detected, returning specialized question set",
		);
	...
	console.log(
		"[AdaptiveLogic] Relevant strengths count:",
	...
	const candidateQuestions = questionBank.filter(
		(q) =>
			q.phase === 2 && q.strengthId && relevantStrengthIds.has(q.strengthId),
	);
	console.log(
		"[AdaptiveLogic] Candidate questions for Phase 2:",
	...
	if (candidateQuestions.length < targetCount) {
		console.log(
			"[AdaptiveLogic] Not enough candidate questions, using all Phase 2 questions",
		);
	...
	console.log(
		"[AdaptiveLogic] Stratified sampling:",
		questionsPerDomain,
	...
	topDomains.forEach((domain, index) => {
		...
		console.log(
			`[AdaptiveLogic] Domain ${domain.domainName}: selected ${selected.length}/${count} questions`,
		);
	});
	...
	if (selectedQuestions.length < targetCount) {
		...
		console.log(
			"[AdaptiveLogic] Filled",
			fillCount,
			"additional questions from remaining candidates",
		);
	}
	...
	console.log(
		"[AdaptiveLogic] Final Phase 2 questions selected:",
...
export function handleEdgeCases(
	...
	if (allEqual && scores.length > 0) {
		console.log(
			"[AdaptiveLogic] All domains equal - presenting broader question set",
		);
