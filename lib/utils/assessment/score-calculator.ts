...
export function calculateDomainScores(
  ...
  const domainScores: Record<string, { totalScore: number; totalWeight: number }> = {};
...
export function calculateStrengthScores(
  ...
  const strengthScores: Record<string, { totalScore: number; totalWeight: number }> = {};
  ...
  for (const answer of answers) {
    ...
    if (question.type === 'CHOICE' && typeof answer.answer === 'string') {
      ...
      if (optionIndex === 0) {
        ...
        strengthScores[ question.strengthId ].totalScore += 0.25 * question.weight;
        strengthScores[ question.strengthId ].totalWeight += question.weight;
      }
    } else if (question.type === 'SCALE') {
      const normalizedValue = normalizeAnswer(answer.answer, question.type);
      strengthScores[ question.strengthId ].totalScore += normalizedValue * question.weight;
      strengthScores[ question.strengthId ].totalWeight += question.weight;
    }
  }
 ...
 */
function calculateConsistency(answers: AnswerData[], questions: QuestionData[]): number {
...
export function calculateConfidenceScore(
  ...
  const relevantQuestions = questions.filter((q) => q.strengthId === strengthId);
...
function processRankingAnswers(
  ...
  for (const answer of answers) {
    const question = questions.find((q) => q.id === answer.questionId);
    if (!question || question.phase !== 3 || question.type !== 'RANKING') continue;
...
export function calculateFinalResults(
  ...
  const rankingBoosts = processRankingAnswers(answers, questions, strengthScores);

  // Combine strength scores with ranking boosts
  const finalScores: Array<{ strengthId: string; score: number; confidence: number }> = [];
  ...
  // Take top 5
  const rankedStrengths: RankedStrength[] = finalScores.slice(0, 5).map((item, index) => {
