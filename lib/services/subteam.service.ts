...
import type { ProjectTypeOption, ProjectType } from '@/lib/types/project-type.types';
...
export async function getSubTeamsList(
  ...
  if (filters?.minMatchScore !== undefined || filters?.maxMatchScore !== undefined) {
    where.matchScore = {
      ...(filters.minMatchScore !== undefined && { gte: filters.minMatchScore }),
      ...(filters.maxMatchScore !== undefined && { lte: filters.maxMatchScore }),
    };
  }
 ...
 */
export async function getSubTeamDetail(subTeamId: string): Promise<SubTeamDetail | null> {
  ...
  const analysis = subTeam.analysis ? JSON.parse(subTeam.analysis as string) : null;
 ...
 */
export async function getTeamMembersForSelector(teamId: string): Promise<SubTeamMember[]> {
...
export async function updateSubTeam(
  ...
  if (input.members) {
    const projectTypeProfile = await prisma.projectTypeProfile.findUnique({
      where: { id: input.projectTypeProfileId || existingSubTeam.projectTypeProfileId },
    });
  ...
  const subTeam = await prisma.subTeam.update({
    ...
    data: {
      ...
      ...(input.projectTypeProfileId && { projectTypeProfileId: input.projectTypeProfileId }),
      matchScore,
      analysis,
    },
  });
 ...
 */
async function getTeamMembersWithStrengths(memberIds: string[]): Promise<SubTeamMember[]> {
  const users = await prisma.user.findMany({
    where: { id: { in: memberIds } },
    select: {
      id: true,
...
export async function getSuggestedMembersForGap(
  ...
  for (const tm of availableMembers) {
    ...
    const fillsGaps = missingStrengths.filter((ms) => memberStrengths.includes(ms));
