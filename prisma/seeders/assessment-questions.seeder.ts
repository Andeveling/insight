...
export async function seedAssessmentQuestions(prisma: PrismaClient) {
  ...
  try {
    ...
    console.log(`   Found ${domains.length} domains and ${strengths.length} strengths`);
    ...
    const questionsToInsert = allAssessmentQuestions.map((q) => {
      ...
      if (q.strength) {
        ...
        if (!strengthId) {
          console.warn(`   ⚠️  Strength "${q.strength}" not found, skipping strength assignment for question ${q.order}`);
        }
      }
    ...
    for (let i = 0; i < questionsToInsert.length; i += batchSize) {
      ...
      console.log(`   Inserted batch ${Math.floor(i / batchSize) + 1}/${Math.ceil(questionsToInsert.length / batchSize)} (${insertedCount}/${questionsToInsert.length} questions)`);
    }

    // Verify insertion
    const totalQuestions = await prisma.assessmentQuestion.count();
    const phase1Count = await prisma.assessmentQuestion.count({ where: { phase: 1 } });
    const phase2Count = await prisma.assessmentQuestion.count({ where: { phase: 2 } });
    const phase3Count = await prisma.assessmentQuestion.count({ where: { phase: 3 } });

    console.log(`   ✅ Successfully seeded ${totalQuestions} assessment questions`);
    console.log(`      Phase 1 (Domain Discovery): ${phase1Count} questions`);
    console.log(`      Phase 2 (Strength Refinement): ${phase2Count} questions`);
    console.log(`      Phase 3 (Ranking Confirmation): ${phase3Count} questions`);
  } catch (error) {
